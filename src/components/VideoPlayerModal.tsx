...
export const VideoPlayerModal: React.FC<VideoPlayerModalProps> = ({
  ...
  const activeSource = React.useMemo(() => {
    ...
    return videoUrl;
  }, [videoUrl, isOffline, streamSourceType, episode, backendUrl, directDownloadUrl]);
  ...
  const handleMediaError = () => {
    setIsLoading(false);
    if (streamSourceType === 'proxy') {
      setStreamSourceType('direct');
      return;
    }
    setHasError(true);
  };
                ...
                <div className="p-6 text-center max-w-lg space-y-4 my-auto">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 text-gray-200 hover:text-white text-xs font-semibold transition-all border border-white/10 cursor-pointer"
                    >
                      <ExternalLink className="w-3.5 h-3.5" />
                      <span>Flux direct externe</span>
                    </a>

                    {/* Copy direct link */}
                    <button
                      onClick={handleCopyLink}
                      className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-300 text-xs font-semibold transition-all border border-white/5 cursor-pointer sm:col-span-2"
                    >
                      {isCopied ? (
                        <>
                        ...
                        <>
                          ...
                          <span>Copier le lien direct du média</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-[11px] font-semibold text-gray-200 border border-white/10 transition-colors"
                    >
